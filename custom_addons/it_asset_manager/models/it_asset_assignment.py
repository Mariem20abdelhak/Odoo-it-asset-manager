...
class ItAssetAssignment(models.Model):
    ...
    date_start = fields.Date(string="Date d'affectation", required=True,
                            default=fields.Date.today, tracking=True)
    date_end = fields.Date(string='Date de retour', tracking=True)
    is_active = fields.Boolean(string='Affectation active',
    ...
    def write(self, vals):
        ...
        if vals.get('date_end'):
            for rec in self:
                rec.asset_id.write({'state': 'available', 'employee_id': False})
        return result
    ...
    def create(self, vals_list):
        ...
        for rec in records:
            rec.asset_id.write({'state': 'assigned', 'employee_id': rec.employee_id.id})
            rec.asset_id.message_post(
                body=_('Asset affecté à %s le %s.') % (rec.employee_id.name, rec.date_start)
            )
        return records
